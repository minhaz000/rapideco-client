...
const Shop = () => {
  ...
  const [query, setQuery] = useState({ categories: [cate], price: [0, 10000], sort: "", s: "", update: false });
  const [pagination, setPagination] = useState({ page: 1, limit: 10 });
  const { data: Categories } = useQueryData(["shop"], `/api/v0/categories?is_delete=false`);
  // prettier-ignore
  ...
  const { data: products, refetch } = useQueryData(["get shop products", pagination, query], formate);
          ...
          <div className="border rounded-md mt-6 p-4">
            ...
            <div className="mt-4">
              <div className="w-full max-w-xs mx-auto mt-4">
                <Slider range min={0} max={10000} value={query.price} onChange={handePrice} />
