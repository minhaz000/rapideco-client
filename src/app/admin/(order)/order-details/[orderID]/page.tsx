...
const OrderDetails = ({ params }: { params: { orderID: string } }) => {
  ...
  const handleDownload = async () => {
    ...
    console.log(`${process.env.NEXT_PUBLIC_SERVER}/api/v0/invoice`);
  ...
  return (
    <>
      <div>
        {/* Download button */}
        <button onClick={handleDoarwnload}>Download PDF</button>
      </div>
    </>
  );
  // return (
  ...
  //         {data?.data && <QRCodeSVG value={`${process.env.NEXT_PUBLIC_HOST}/track/${data?.data._id}`} />}
  ...
  //             <span className="bg-green-500 text-white px-2 py-1 rounded text-[10px]">{data?.data.status}</span>
  //           ) : data?.data.status == "canceled" ? (
  //             <span className="bg-red-500 text-white px-2 py-1 rounded text-[10px]">{data?.data.status}</span>
  //           ) : (
  //             <span className="bg-orange-500 text-white px-2 py-1 rounded text-[10px]">{data?.data.status}</span>
  //           )}
  //         </div>
  //         <div className="flex items-center justify-between gap-4 mb-2">
  ...
  //         <span>Total :</span>
  //         <span className="text-2xl font-semibold">{data?.data.payment_info.amount} ৳</span>
  //       </div>
